...
// Print( "" + get_errorcode(GetLastError()) );
string get_errorcode(int error_code){
   string server_error = TradeServerReturnCodeDescription(error_code);
   string more_error = ErrorDescription(error_code);
   string concat = server_error + more_error;
   
   if( (server_error !="") || (more_error != "" )){
      return(concat);
   }
